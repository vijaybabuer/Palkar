var storyItemController = function(sb, input){
	var relPathIn=input.relPath, textReactionShowAllButton=null, authenticatedUser=input.authenticatedUser, storyItemList=null, albumContainerJS=null, storyPage=input.storyPage, currentNavBtn=null, prevNavBtn=null, scrollListContainer=null,
		loginUserName=input.username, lastUpdatedStreamDate = input.lastUpdatedStreamDate, getMoreStories=input.getMoreStories, numberOfStoriesToGet=input.numberOfStoriesToGet, storiesDivId = input.storiesDivId, documentTypeToShow="ALL", contactTypeToShow="ANY",
		storyJSTemplateName = input.storyJSTemplateName, newestUpdatedStreamDate = input.newestUpdatedStreamDate, overRideHomeUrl = input.overRideHomeUrl, gettingNewerStories = false, lastScrollTop = sb.dom.find(window).scrollTop();
   ...
   function _scrollToReactionsSection(e){
	...
	function _showNewerStories(data){	
	   ...
	   function _showMoreStoriesClicked(e){
		   ...
		   sb.utilities.postV2(relPathIn+'stream?mediaType=json', {username: loginUserName, lastUpdatedStreamDate: lastUpdatedStreamDate, streamSize: numberOfStoriesToGet, postedBeforeAfter: "BEFORE"}, _showStories);		   
	   }
	   
	   function _getNewerStories(){
		   sb.utilities.postV2(relPathIn+'stream?mediaType=json', {username: loginUserName, lastUpdatedStreamDate: newestUpdatedStreamDate, streamSize: numberOfStoriesToGet, postedBeforeAfter: "AFTER"}, _showNewerStories);
	   }
	   ...
	   function _storiesDivScroll(e){
		   ...
		   if(currentScrollTop > lastScrollTop){
			   ...
			   if(sb.dom.find('#showMore').visible() && !sb.dom.find('#showMore').is(":disabled")){
   ...
   return{
	   init:function() {
       	try{
       			sb.utilities.trace('initializing module: storyItemController');        			
	    		...
	    		sb.dom.find('.unSharePageButton').bind('click', _unSharePageButtonClickEvent);
	    		sb.dom.find('#storiesDivHeader').find('#format').buttonset();
	    		sb.dom.find('#storiesDivHeaderNonMobile').find('#formatNonMobile').buttonset();	    		
